...
- [ ] 🆕 `turn_context.naval` lists **red's own movable ship groups** (id, pos, `move_range_nm`~80, `threat_nm`?). `POST /retribution-ai/naval/move` `{"side":"red","ship_id":"<id from naval>","lat":<lat>,"lng":<lng>}` with a point a few nm away over water → ✅ ok; re-reading `naval` shows that group's `destination`. 🔴 A point >80 nm away → `ok:false` (range); a point across land → `ok:false` (over land); a **blue** ship id → `ok:false` (not yours). Omit `lat`/`lng` → cancels the pending move (`destination` clears). After a Take Off + turn, the group has actually moved on the map.
