...
* **[FIX]** Two of the fork's own Splash Damage settings had never done anything: the parked-aircraft damage boost and the anti-radiation ship-radar kill called a `getAGL()` this script does not define, so they threw on every blast wave, and the cluster bomblet reduction was written to a key spelled differently from the one the script reads.
