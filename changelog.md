...
* **[Mission Generator]** An escort asked to arrive ahead of its package now does. It used to spend the head start orbiting the join and cross the target with the package anyway, because the DCS escort task ties it to the flight it protects from the join onwards; that task is now held back until the protected flight reaches its ingress point, so the escort flies its own route until then and still covers the attack and the way home. Escort and SEAD Escort both; a negative TOT offset is the whole of the ask.
