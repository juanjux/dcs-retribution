...
def _cruise_missiles_remaining(game: Game, tgo) -> int | None:
    ...
    try:
        from game.cruise_raids import (  # type: ignore[import-not-found]
            tgo_magazines,  # fork feature, absent in the base game
        )
    except ImportError:
        return None
...
def _tgo_repairables(tgo) -> tuple[int, list]:
    ...
    if isinstance(tgo, BuildingGroundObject):
        ...
        if not dead:
            return 0, []
        try:
            cost = int(tgo.repair_cost())  # type: ignore[attr-defined]
        except Exception:
            cost = 0  # no repair feature in this build, or nothing to price
        if cost <= 0:
            return 0, []
...
def build_settings(game: Game) -> SettingsView:
    # Some of what the planner likes to know is a fork/extension feature the base game
    # may not have (repair timers, cruise-missile raids, the crash-losses doctrine).
    # Read them defensively so this runs unchanged on a build without them: an absent
    # feature reports its "off" value rather than crashing the whole settings call.
    try:
        from game.theater.controlpoint import (  # type: ignore[attr-defined]
            RUNWAY_REPAIR_TURNS,
        )
    except ImportError:
        RUNWAY_REPAIR_TURNS = 0
    ...
    return SettingsView(
        ...
        crashes_dont_count=getattr(s, "ignore_non_combat_air_losses", False),
        ...
        cruise_missile_strikes=getattr(s, "cruise_missile_strikes", False),
        cruise_missile_auto_raids=getattr(s, "cruise_missile_auto_raids", False),
    )
