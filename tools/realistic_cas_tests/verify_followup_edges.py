...
def run(root,file,setup,event,expected,forbidden=None):
    with zipfile.ZipFile(root/file) as z: text=z.read("mission").decode()
    rt=LuaRuntime()
    rt.execute(text)
    rt.execute(MOCK)
    rt.execute(setup)
    ...
    if forbidden: assert forbidden not in logs
...
def main(root):
    run(root,"21_scope_unit.miz",
        "for _,g in pairs(_groups) do g.controller.isTargetDetected=function() return false end end",
        "","INVALID: no adquirio todos", "op=invisible")
    run(root,"24_memory_hidden.miz",
    ...
    run(root,"26_CAS_revoke.miz",
        "for _,u in pairs(_units) do u.getAmmo=function() return {} end end",
        SHOT,"INVALID: sin Kh-29T restante", "op=invisible")
    run(root,"30_AttackUnit_always_hidden.miz","",SHOT,"RESULTADO: LANZO CONTRA OCULTO")
    def coalition(n):
        path=next(root.glob(n+"_*.miz"))
        with zipfile.ZipFile(path) as z: return lua.loads(z.read("mission").decode())["mission"]["coalition"]
