...
def main():
    ...
    aircraft = b.plane('RADAR_HORNET', x=X+18000, z=Z, kind='FA-18C_hornet', size=1,
                       altitude=4500, speed=150, pod=False, side='blue', hold=True,
                       tasks=[zone(x=X,z=Z)])
    wp = aircraft['route']['points'][1]
    wp['alt_type'] = 'BARO'
    tasks = wp['task']['params']['tasks']
    tasks[len(tasks)+1] = {'id':'Orbit','enabled':True,'auto':False,'number':len(tasks)+1,
        'params':{'pattern':'Circle','speed':150,'altitude':4500,'point':{'x':X+18000,'y':Z}}}
    aircraft['route']['points'] = {1:wp}
    aircraft['units'][1]['alt_type'] = 'BARO'
    cfg = {'observer':aircraft['units'][1]['name'], 'targets':[], 'groups':[]}
    for name,x,z,kind in [('MOVING_FAR',X-2000,Z-300,'moving'),
        ...
        g['route']['points'] = {1:p}
        if kind=='moving':
            p.update(speed=6,speed_locked=True,action='Off Road',type='Turning Point')
            destination=copy.deepcopy(p)
            destination.update(x=x+8000,y=z,ETA_locked=False,task={'id':'ComboTask','params':{'tasks':{}}})
            g['route']['points'][2]=destination
        cfg['targets'].append({'name':g['units'][1]['name'],'groupName':name,'kind':kind})
    ...
    script='RCAS_RADAR_TEST='+literal(cfg)+'\n'+'\n'.join(p.read_text(encoding='utf-8') for p in sources)
    b.m['trig']={'actions':{1:'a_do_script('+literal(script)+'); mission.trig.func[1]=nil;'},
    ...
    b.m['trigrules']={1:{'actions':{1:{'predicate':'a_do_script','text':script,'KeyDict_text':script}},
        'rules':{1:{'predicate':'c_time_after','seconds':3}},'predicate':'triggerOnce','eventlist':'',
        'comment':b.name,'colorItem':'0xffffffff'}}
    ...
    (args.metadata/'manifest.json').write_text(json.dumps({
        'file':str(args.output),'sha256':hashlib.sha256(args.output.read_bytes()).hexdigest(),
        'seed':str(args.seed),'seed_sha256':hashlib.sha256(args.seed.read_bytes()).hexdigest(),
        'sources':{str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest()
                   for p in sources+[Path(__file__),HERE/'build_missions.py']},'config':cfg},indent=2),encoding='utf-8')
