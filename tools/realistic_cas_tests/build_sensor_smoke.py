...
def main():
    ...
    def target(name, x, z, expect):
        ...
        cfg['targets'].append({'name': g['units'][1]['name'], 'groupName': name, 'expect': expect})

    for lane, kind, near, far in ((0, 'T-72B', 2000, 4000), (1, 'M-1 Abrams', 4000, 6000)):
        z = Z+lane*12000
        g = b.vehicle('GROUND_'+str(lane), x=X, z=z, kind=kind, side='blue')
        cfg['observers'].append({'name': g['units'][1]['name'], 'typeName': kind, 'category': 'ground'})
    ...
    cfg['observers'].append({'name': g['units'][1]['name'], 'typeName': 'T-72B', 'category': 'ground'})
    target('CITY_HIDDEN', X+1500, z, False)
    cfg['zones'].append({'x': X+1500, 'z': z, 'radius': 1500, 'cover': 'city'})

    for lane, kind, name, pod in ((1, 'F-15C', 'VISUAL_F15', False),
                                   (2, 'A-10C_2', 'POD_A10', True),
                                   (3, 'FA-18C_hornet', 'RADAR_F18', False)):
        z = Z+lane*40000
        g = b.plane(name, z=z, kind=kind, size=1, altitude=2000, pod=pod,
                    side='blue', hold=True, tasks=[zone(x=X,z=z)])
        cfg['observers'].append({'name': g['units'][1]['name'], 'typeName': kind,
                                'category': 'air', 'role': 'CAS', 'equipment': {'targetingPod': pod}})
    ...
    script = 'RCAS_SENSOR_TEST='+literal(cfg)+'\n'+'\n'.join(p.read_text(encoding='utf-8') for p in sources)
    ...
    manifest={'file':path.name,'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),
